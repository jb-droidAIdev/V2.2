        ...
        <AnimatePresence>
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                ...
                >
                        ...
                        <div className="flex items-center gap-6">
                            ...
                            <div>
                                <div className="flex items-center gap-3">
                                    <h2 className="text-3xl font-black text-white tracking-tight">Audit Details</h2>
                                    <span className={cn(
                                ...
                                <p className="text-slate-500 text-sm font-medium">Session ID: {previewTarget.id}</p>
                            </div>
                        </div>
                        <button
                            onClick={onClose}
                    ...
                    <div className="p-8 overflow-y-auto custom-scrollbar space-y-8">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-white/[0.02] p-6 rounded-3xl border border-white/5">
                            <div>
                                ...
                                <p className="text-base font-bold text-white font-mono">{previewTarget.sampledTicket?.ticket?.externalTicketId || previewTarget.ticketReference || 'N/A'}</p>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <p className={cn("text-2xl font-black", previewTarget.score >= 90 ? "text-emerald-400" : previewTarget.score >= 70 ? "text-amber-400" : "text-rose-400")}>{previewTarget.score}%</p>
                            </div>
                        </div>

                        {/* Evaluation Form */}
                        <div className="space-y-6">
                            <h3 className="text-sm font-black uppercase tracking-[0.2em] text-slate-400">Evaluation Breakdown</h3>
                            <div className="space-y-4">
                                {previewTarget.formVersion?.criteria?.map((criterion: any) => {
                                    const scoreObj = previewTarget.scores?.find((s: any) => s.criterionId === criterion.id);
                                    const isYes = scoreObj?.score === criterion.weight;
                                    const isNA = scoreObj?.score === -1;

                                    return (
                                        <div key={criterion.id} className="bg-white/[0.02] border border-white/5 rounded-3xl p-6">
                                            <div className="flex flex-col md:flex-row gap-6">
                                                <div className="flex-1 space-y-2">
                                                    <div className="flex items-center gap-3">
                                                        <span className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-xs font-bold text-slate-400">
                                                            {criterion.orderIndex + 1}
                                                        </span>
                                                        <h4 className="text-sm font-bold text-slate-200">{criterion.title}</h4>

                                                        {/* Challenge Toggle for Supervisors/Admins */}
                                                        {/* Challenge Toggle for Supervisors/Admins */}
                                                        {!isYes && !isNA && !disputeInfo && (userRole === 'ADMIN' || userRole === 'OPS_TL' || userRole === 'QA_TL') && (
                                                            <button
                        ...
                        {disputeInfo && (
                            <div className="space-y-4 animate-in slide-in-from-bottom-2 duration-500 border-t border-white/5 pt-8">
                                <div className="flex items-center justify-between">
                                    <h3 className="text-xs font-black uppercase tracking-[0.2em] text-rose-400 flex items-center gap-2">
                                        <History className="w-4 h-4" /> Dispute History
                                    </h3>
                                    <span className="px-3 py-1 bg-rose-500/10 text-rose-400 text-[10px] font-black uppercase rounded-full border border-rose-500/20">
                                        {disputeInfo.status.replace(/_/g, ' ')}
                                    </span>
                                </div>
                                ...
                                <div className="space-y-4">
                                    {disputeInfo.items.map((item: any) => (
                                        <div key={item.id} className="bg-white/[0.02] border border-white/5 rounded-3xl p-6 space-y-4">
                                            <div className="flex items-center justify-between">
                                                ...
                                                {item.finalVerdict ? (
                                                    <span className={cn("text-[10px] font-black uppercase px-2 py-0.5 rounded", item.finalVerdict === 'ACCEPTED' ? "bg-emerald-500/10 text-emerald-400" : "bg-rose-500/10 text-rose-400")}>
                                                        Final: {item.finalVerdict}
                                                    </span>
                                                ) : item.qaVerdict && (
                                                    <span className={cn("text-[10px] font-black uppercase px-2 py-0.5 rounded", item.qaVerdict === 'ACCEPTED' ? "bg-emerald-500/10 text-emerald-400" : "bg-rose-500/10 text-rose-400")}>
                                                        QA: {item.qaVerdict}
                                                    </span>
                                                )}
                                            </div>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <div className="bg-black/20 p-3 rounded-2xl text-[11px] text-slate-300 border border-white/5">
                                                        {item.reason}
                                                    </div>
                                                </div>
                                                ...
                                                {item.qaComment && (
                                                    <div className="space-y-2">
                                                        ...
                                                        <div className="bg-blue-500/5 p-3 rounded-2xl text-[11px] text-slate-300 border border-blue-500/10">
                                                            {item.qaComment}
                                                        </div>
                                                    </div>
                                                )}
                                                ...
                                                {item.reappealReason && (
                                                    <div className="space-y-2">
                                                        ...
                                                        <div className="bg-amber-500/5 p-3 rounded-2xl text-[11px] text-slate-300 border border-amber-500/10">
                                                            {item.reappealReason}
                                                        </div>
                                                    </div>
                                                )}
                                                ...
                                                {item.finalComment && (
                                                    <div className="space-y-2">
                                                        ...
                                                        <div className="bg-emerald-500/5 p-3 rounded-2xl text-[11px] text-slate-300 border border-emerald-500/10">
                                                            {item.finalComment}
                                                        </div>
                                                    </div>
                                                )}
                                            ...
                                            <div className="flex gap-2 pt-2 border-t border-white/5">
                                                ...
                                                {!item.qaVerdict && userRole === 'QA' && previewTarget.auditorId === localStorage.getItem('user_id') && (
                                                    <>
                                                        <button onClick={() => onVerdict(item.id, 'ACCEPTED', 'qa')} className="px-4 py-2 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 text-[10px] font-black uppercase rounded-xl transition-all flex items-center gap-2">
                                                            <CheckCircle className="w-3.5 h-3.5" /> Accept Mistake
                                                        </button>
                                                ...
                                                {/* Admin Final Verdict Stage Actions */}
                                                {!item.finalVerdict && item.reappealReason && userRole === 'ADMIN' && (
                                                    <>
                                    ...
                                    {disputeInfo.status === 'QA_REJECTED' && (userRole === 'ADMIN' || userRole === 'OPS_TL') && (
                                        <button
                                            onClick={onReappeal}
                    ...
                    {/* Footer */}
                    <div className="p-8 border-t border-white/5 bg-black/60 flex items-center justify-between shrink-0">
                        {isFilingDispute ? (
                            <>
                                <button
                                    onClick={() => setIsFilingDispute(false)}
                                    className="px-8 py-4 bg-white/5 hover:bg-white/10 text-white text-xs font-black uppercase tracking-[0.2em] rounded-2xl transition-all border border-white/10"
                                >
                                    Cancel filing
                                </button>
                                <div className="flex items-center gap-6">
                                    ...
                                    <button
                                        ...
                                        className="px-12 py-4 bg-rose-600 hover:bg-rose-500 text-white text-xs font-black uppercase tracking-[0.2em] rounded-2xl transition-all shadow-xl shadow-rose-600/20"
                                    >
                                        Raise Dispute
                                    </button>
                                </div>
                            ...
                            <>
                                ...
                                <button
                                    onClick={onClose}
                                    className="px-12 py-4 bg-blue-600 hover:bg-blue-500 text-white text-xs font-black uppercase tracking-[0.2em] rounded-2xl transition-all shadow-xl shadow-blue-600/20"
                                >
                                    Close Review
                                </button>
                            </>
