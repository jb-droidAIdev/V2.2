...
import {
  CalibrationSessionStatus as PrismaSessionStatus,
  CalibrationTicketType as PrismaTicketType,
  CalibrationAnchorStatus as PrismaAnchorStatus,
...
export class CalibrationService {
  ...
  async randomizeAllTickets(sessionId: string) {
    ...
    await this.randomizeReproducibility(
      sessionId,
      session.reproducibilityTicketCount,
    );

    // Step 2: Randomize Repeatability tickets
    await this.randomizeRepeatability(
      sessionId,
      session.repeatabilityTicketCount,
    );
  ...
  private async randomizeReproducibility(sessionId: string, count: number) {
    ...
    const audits = await this.prisma.audit.findMany({
      ...
      select: {
        ...
        sampledTicket: {
          select: {
            ticket: { select: { externalTicketId: true, metadata: true } },
          },
        },
    ...
    if (audits.length < count) {
      throw new BadRequestException(
        `Not enough audits available for reproducibility. Need ${count}, found ${audits.length}`,
      );
    }
    ...
    await this.prisma.calibrationTicket.createMany({
      data: selectedAudits.map((audit: any) => ({
        ...
        auditId: audit.id,
        ticketId:
          audit.sampledTicket?.ticket?.externalTicketId ||
          audit.ticketReference ||
          'UNKNOWN',
  ...
  private async randomizeRepeatability(sessionId: string, count: number) {
    ...
    if (audits.length < count) {
      throw new BadRequestException(
        `Not enough audits available for repeatability. Need ${count}, found ${audits.length}`,
      );
    }
    ...
    for (const audit of selectedAudits) {
      ...
      const ticketId =
        (audit as any).sampledTicket?.ticket?.externalTicketId ||
        (audit as any).ticketReference ||
        'UNKNOWN';
      ...
      ticketsToCreate.push({
        ...
        metadata:
          (audit as any).sampledTicket?.ticket?.metadata ||
  ...
  private async randomizeAccuracy(sessionId: string) {
    ...
    if (
      selectedHigh.length < 2 ||
      selectedMid.length < 2 ||
      selectedLow.length < 2
    ) {
      throw new BadRequestException(
        'Not enough audits in each score range for accuracy calibration',
      );
    }
  ...
  async validateAnchor(
    ...
    if (!participant) {
      throw new ForbiddenException(
        'You are not a participant in this calibration session',
      );
    }
    ...
    const bothResponded =
      updatedAnchor.qaTlApproved !== null &&
      updatedAnchor.amSdmApproved !== null;
  ...
  private async replaceAnchor(
    ...
    if (!replacementAudit) {
      throw new BadRequestException(
        `No replacement anchor found for ${scoreRange} range`,
      );
    }
  ...
  private async checkAndOpenScoring(sessionId: string) {
    ...
    const allValidated = anchors.every(
      (a: CalibrationAnchor) => a.status === PrismaAnchorStatus.VALIDATED,
    );

    if (allValidated && anchors.length >= 6) {
      // Create accuracy calibration tickets from validated anchors
      const validatedAnchors = anchors.filter(
        (a: CalibrationAnchor) => a.status === PrismaAnchorStatus.VALIDATED,
      );
  ...
  private async checkParticipantCompletion(
    ...
    const hasCompletedAll = allTicketIds.every((id: string) =>
      scoredTicketIds.has(id),
    );
  ...
  private async calculateRepeatability(sessionId: string): Promise<number> {
    ...
    return avgRaterDeltas.length > 0
      ? avgRaterDeltas.reduce((sum, val) => sum + val, 0) /
          avgRaterDeltas.length
      : 0;
  }
    ...
    for (const participant of participants) {
      ...
      await this.prisma.calibrationResult.create({
        data: {
          ...
          avgAccuracyGap,
          passedAccuracy:
            avgAccuracyGap !== null && avgAccuracyGap < session.targetAccuracy,
        },
      });
    }
  }
