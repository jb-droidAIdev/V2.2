  ...
  async runSampling(batchId: string, campaignId: string) {
    ...
    const run = await this.prisma.samplingRun.create({
      data: {
        ...
        configUsed: {
          samplingRate,
          stratification: campaign.stratification ?? {},
        },
      },
    });
    ...
    if (qas.length === 0) {
      this.logger.warn(
        'No active QAs found for campaign. Sampling will leave assignments empty.',
      );
    }
    ...
    this.logger.log(
      `Sampling complete. Selected ${sampledTicketsToCreate.length} tickets from ${tickets.length}.`,
    );
